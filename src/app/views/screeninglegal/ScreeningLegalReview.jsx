...
class ScreeningLegalReview extends React.Component {


    handleSubmit = event => {
        var sr = `{"screening_l_request_data": ` + this.fetchScreeningRequestData() +
            `,"screening_l_related_person": [` + this.fetchScreeningRelatedPerson() + `]` +
            `,"screening_l_related_entity": [` + this.fetchScreeningRelatedEntity() + `]` + 
            `,"screening_l_attachment": [` + this.fetchScreeningDocument() + `]`+
            `}`;
        alert(sr);
    ...
    fetchScreeningRequestData() {
        var screening_n_request_data = JSON.parse(localStorage.getItem('screening_l_request_data'));
        var data = "";
    ...
    fetchScreeningRelatedPerson() {
        // alert("inside P");
        var screening_l_related_person = JSON.parse(localStorage.getItem('screening_n_related_person'));
        var data = "";
        if (screening_l_related_person) {
            for (var i = screening_l_related_person.length; i > 0; i--) {
                if (i != 1) {
                    data += `{"related_person_request_data":` + JSON.stringify(screening_l_related_person[i - 1]) + "},";
                } else {
                    data += `{"related_person_request_data":` + JSON.stringify(screening_l_related_person[i - 1]) + "}";
                }
            }
    ...
    fetchScreeningRelatedEntity() {
        // alert("inside");
        var screening_l_related_entity = JSON.parse(localStorage.getItem('screening_n_related_entity'));
        var data = "";
        if (screening_l_related_entity) {
            for (var i = screening_l_related_entity.length; i > 0; i--) {
                if (i != 1) {
                    data += `{"related_entity_request_data":` + JSON.stringify(screening_l_related_entity[i - 1]) + "},";
                } else {
                    data += `{"related_entity_request_data":` + JSON.stringify(screening_l_related_entity[i - 1]) + "}";
                }
            }
    ...
    render() {
        ...
        return (
            ...
            <div style={divStyle}>
                <ScreeningTabTitle />
                <h2 style={{ color: 'lightBlue', textAlign:'center' }}><span>Review Legal Screening!!!</span></h2>
                <hr />
...
ScreeningLegalReview.propTypes = {
